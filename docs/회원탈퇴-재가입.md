...
- **마이페이지 > 회원탈퇴**에서 비밀번호 확인 후 탈퇴 시:
  - **Firebase Auth**: 해당 이메일 계정이 있으면 로그인 후 `user.delete()`로 **삭제** (재가입 시 이메일 중복 방지).
  - **Auth에 계정이 없는 구 가입** 등은 `members`에 저장된 비밀번호와 입력값이 일치할 때만 Firestore 탈퇴 처리.
  - Firestore `members` 문서는 **삭제하지 않음**. 아래 필드 **업데이트**:
    - `status: 'withdrawn'`
...
| 파일 | 역할 |
|------|------|
| `js/mypage-api.js` | `withdrawMember(docId, password)` – Auth 삭제(가능 시) 후 status/withdrawnAt 업데이트 |
